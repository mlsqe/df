...
<div class="layui-form" style="margin-bottom:10px;">
    <div class="layui-inline">
        <input type="text" id="userKeyword" placeholder="用户名/手机号" class="layui-input" style="width:160px;">
    </div>
    <div class="layui-inline">
        <select id="userStatus">
            ...
            <option value="pending_booster">预备打手</option>
        </select>
    </div>
    <div class="layui-inline">
        <select id="userRole">
...
<script type="text/html" id="userRoleTpl">
    {{# if(d.role === 'super_admin'){ }}
        <span class="layui-badge layui-bg-red">超级管理员</span>
    {{# } else if(d.role === 'admin'){ }}
        <span class="layui-badge layui-bg-blue">管理员</span>
    {{# } else if(d.role === 'booster'){ }}
        <span class="layui-badge layui-bg-green">打手</span>
    {{# } else { }}
        <span class="layui-badge layui-bg-cyan">玩家</span>
    {{# } }}
</script>
    ...
    layui.use(['table', 'layer', 'form', 'dropdown'], function(){
        ...
        table.render({
            ...
            cols: [[
                ...
                {field:'real_name', title:'姓名', templet: function(d){return d.real_name||'-';}},
                {field:'phone', title:'手机', templet: function(d){return d.phone||'-';}},
                {field:'wechat', title:'微信', templet: function(d){return d.wechat||'-';}},
                {field:'role', title:'角色', templet: '#userRoleTpl'},
                {field:'status', title:'状态', templet: function(d){
                    if (d.pending_booster == 1 || d.pending_booster == '1') {
                        return '<span class="layui-badge layui-bg-orange">预备打手</span>';
                    }
                ...
                {field:'balance', title:'余额', templet: '<span>¥{{d.balance}}</span>'},
                {field:'created_at', title:'注册时间', templet: '<div>{{layui.util.toDateString(d.created_at, "yyyy-MM-dd HH:mm:ss")}}</div>'},
                        ...
                        if(action === 'detail'){
                            ...
                            .then(res=>{
                                if(res.code===0 || res.success){
                                    ...
                                    var html = '<div style="padding:10px;"><p>用户名: '+u.username+'</p><p>姓名: '+(u.real_name||'-')+'</p><p>手机: '+(u.phone||'-')+'</p><p>微信: '+(u.wechat||'-')+'</p><p>角色: '+u.role+'</p><p>状态: '+u.status+'</p><p>余额: ¥'+u.balance+'</p><p>下单数: '+(u.order_count||0)+'</p><p>打单数: '+(u.boost_count||0)+'</p><p>注册时间: '+u.created_at+'</p><hr>打手信息：'+(u.booster ? '<p>等级: '+(u.booster.level_name||'无')+'</p><p>保证金: ¥'+u.booster.deposit_paid+'</p><p>评分: '+u.booster.rating+'</p><p>完成订单: '+u.booster.total_orders+'</p>' : '非打手')+'</div>';
                            ...
                            layer.open({
                                ...
                                content: '<div style="padding:20px;" class="layui-form">' +
                                    '<div class="layui-form-item">' +
                                        ...
                                        '<div class="layui-input-block"><input type="text" id="edit_real_name" value="' + (data.real_name||'') + '" class="layui-input"></div>' +
                                    '</div>' +
                                    '<div class="layui-form-item">' +
                                        '<label class="layui-form-label">手机号</label>' +
                                        '<div class="layui-input-block"><input type="text" id="edit_phone" value="' + (data.phone||'') + '" class="layui-input"></div>' +
                                    '</div>' +
                                    '<div class="layui-form-item">' +
                                        '<label class="layui-form-label">微信号</label>' +
                                        '<div class="layui-input-block"><input type="text" id="edit_wechat" value="' + (data.wechat||'') + '" class="layui-input"></div>' +
                                    '</div>' +
                                    '<button class="layui-btn layui-btn-fluid" onclick="saveUserEdit(' + data.id + ')">保存</button>' +
                                '</div>',
                                ...
                                content: '<div style="padding:20px;" class="layui-form">' +
                                    '<div class="layui-form-item">' +
                                        ...
                                        '<div class="layui-input-block">' +
                                            '<input type="number" id="balanceAmount" step="0.01" class="layui-input" placeholder="正数充值，负数扣除">' +
                                        '</div>' +
                                    '</div>' +
                                    '<div class="layui-form-item">' +
                                        '<label class="layui-form-label">备注</label>' +
                                        '<div class="layui-input-block">' +
                                            '<input type="text" id="balanceRemark" class="layui-input" placeholder="可选">' +
                                        '</div>' +
                                    '</div>' +
                                    '<button class="layui-btn layui-btn-fluid" onclick="adjustBalance(' + data.id + ')">确认修改</button>' +
                                '</div>',
                                success: function(){ form.render(); }
                            });
                        } else if(action === 'ban'){
        ...
        window.saveUserEdit = function(userId){
            var realName = $('#edit_real_name').val().trim();
            var phone = $('#edit_phone').val().trim();
            var wechat = $('#edit_wechat').val().trim();
            if (!realName && !phone && !wechat) {
                Toast.warning('姓名、手机号和微信号不能都为空');
                return;
            }
            fetch('api/admin.php', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({action:'update_user', user_id: userId, real_name: realName, phone: phone, wechat: wechat})
            })
            .then(r => r.json())
            .then(res => {
                if(res.code === 0 || res.success){
        ...
        window.adjustBalance = function(userId){
            ...
            var remark = $('#balanceRemark').val().trim();
            fetch('api/admin.php', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({action:'balance_adjust', user_id: userId, amount: amount, remark: remark})
            })
            .then(r => r.json())
            .then(res => {
                if(res.code === 0 || res.success){
