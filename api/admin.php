...
switch ($action) {
    ...
    case 'delete_user':
        ...
        try {
            ...
            $boosterId = $db->prepare("SELECT id FROM boosters WHERE user_id = ?")->execute([$uid])->fetchColumn();
            if ($boosterId) { $db->prepare("UPDATE orders SET booster_id = NULL WHERE booster_id = ?")->execute([$boosterId]); }
