            ...
            <div class="layui-tab-content">
                ...
                <div class="layui-tab-item">
                    <form class="layui-form" id="registerForm" onsubmit="return false;">
                        <!-- 修复：让 label 和 select 并排显示，确保文字完整 -->
                        <div class="layui-form-item" style="display:flex; align-items:center;">
                            <label class="layui-form-label reg-role-label">注册身份</label>
                            <div class="layui-input-block reg-role-block" style="flex:1;">
                        ...
                        <div id="boosterSection" style="display:none;">
                            ...
                            <!-- 模式内容：四种情况都由 PHP 决定显示哪个区块 -->
                            <div id="boosterModeWrapper">
                                <?php if ($boosterMode === 'invite'): ?>
                                <div class="layui-form-item">
                                    <div class="layui-input-wrap">
                                        <div class="layui-input-prefix"><i class="layui-icon layui-icon-vercode"></i></div>
                                        <input type="text" name="invite_code" placeholder="请输入打手邀请码" class="layui-input">
                                    </div>
                                </div>
                                <?php elseif ($boosterMode === 'deposit'): ?>
                                <div class="reg-tip" style="background:#FFF8F0; padding:10px; border-radius:8px; text-align:center;">
                                    ⚠️ 注册打手需缴纳保证金 <strong style="color:#FF5722;">¥<?= $depositAmount ?></strong>
                                </div>
                                <?php elseif ($boosterMode === 'invite_or_deposit'): ?>
                                ...
                                <div id="modeContentDeposit" style="display:none;">
                                    <div class="reg-tip" style="background:#FFF8F0; padding:10px; border-radius:8px; text-align:center;">
                                        ⚠️ 注册打手需缴纳保证金 <strong style="color:#FF5722;">¥<?= $depositAmount ?></strong>
                                    </div>
                                </div>
                                <?php else: ?>
                                <div class="reg-tip" style="background:#F0FFF4; padding:10px; border-radius:8px; text-align:center;">
                                    ✅ 自由注册成功后将进入待审核状态
                                </div>
                                <?php endif; ?>
                            </div>
...
<script>
    ...
    layui.use(['form', 'element', 'layer'], function() {
        ...
        // 如果是 invite_or_deposit 模式，监听 radio 切换
        form.on('radio(user_select_mode)', function(data) {
            var inviteDiv = document.getElementById('modeContentInvite');
            var depositDiv = document.getElementById('modeContentDeposit');
            if (!inviteDiv || !depositDiv) return;
